...
Task ID: 29
Agent: opencode (GLM-5.3-Flash, local)
Task: P5 — PWA Phase B: hand-rolled service worker, offline fallback, dev-guarded registration. (Phase A — manifest.webmanifest, icon set 192/512 + maskable, apple-touch-icon, theme-color — had already shipped with the P1 SEO work; verified rather than rebuilt.)

Work Log:
- public/sw.js (plain JS, ~5.6KB): caching contract is narrow by design — /api/* is passed through untouched (never cached, not even on failure, non-negotiable); /_next/static + /icons + favicon/logo/manifest are cache-first (immutable); /uploads/* stale-while-revalidate; GET navigations network-first with successful /product/* pages retained in a bounded pages cache (30 entries, oldest evicted) for offline replay; every non-GET and cross-origin request untouched. Failure ladder for navigations: cached copy of that exact URL → /offline.html → bare 503.
- Deploy-safety mechanics (the footguns from the plan review): no self.terminating() (not a real API — removed from the earlier brief); skipWaiting + clients.claim only; activate keeps current + previous generation of each cache family (ek-static/ek-pages/ek-uploads) and deletes N-2 and older, so tabs opened before a deploy keep working (their hashed chunks are gone from the server after deploy); VERSION constant bumped alongside package.json on every shell-touching deploy.
- Offline scope held honest: previously visited product pages replay from the bounded cache (SSR HTML from Task 26 carries full label data); home/queue/submit are NOT faked offline. Mutations (submit/review/logout) that fail at the network layer now throw a clear Swedish message from apiFetch ("Ingen nätverksanslutning — …") instead of the browser's "Failed to fetch"/"Load failed" TypeError text; the SW never queues or replays POSTs.
...
- Verification: 124/124 tests (448 expects), eslint clean, tsc clean in src/ (only pre-existing examples/ noise). Production build compiles; local note — the build script's `cp -r` fails on this Windows box (bun shell lacks cp -r; Railway's Linux build is unaffected) — worked around locally with Copy-Item. Local standalone server smoke: /sw.js 200 + no-cache, /offline.html 200, /api health 200 (after db push against the local prod.db copy), home SSR 200, unknown barcode → real 404.
- Deployed 7cc4178 and verified in production: real sw.js live (application/javascript, no-cache, content-matched — first poll returned the SPA shell at 200 text/html, the exact false-positive trap from Task 25-b; marker is content-match, not status), /offline.html 200 with Swedish copy, manifest + /api + home all healthy.
...
---
Task ID: 30-i (Task 30 Phase 30G — release gates + release note)
Agent: opencode (GLM-5.3-Flash, local)
Task: 30G from the implementation brief: final gates with real counts, production deploy + probes, release note with rollback, honest-limits record. Complete Task 30.

Release note:
- RELEASE CANDIDATE: main @ d4e8ee6 (30F), preceded by 30A e8bf3d0, 30B f23b5a2, 30C 7f50468, 30D 8bbb18e, 30E 7ebee90, plus 30-b/30-c prep. All pushed; Railway auto-deployed; production verified live at the 30F commit.
- Versions: package.json / APP_VERSION 0.2.1 → 0.3.0; sw.js cache generation v0.2.2 → v0.3.0 (bumped in 30A because client deps changed — the audit's explicit exception to the no-bump rule). next 16.1.1 → 16.3.4, eslint-config-next 16.3.4, react/react-dom 19.0.0 → 19.2.8, sharp 0.34.3 → 0.35.4 (runtime-verified). Removed unused: recharts, next-auth, next-intl, ui/chart.tsx (last dangerouslySetInnerHTML gone with it). Bun 1.4.0 / Node 24.20.0 locally; Railway runtime not directly inspectable from here.
- Changed boundaries: every JSON route (streaming byte caps, runtime type guards, deliberate 400/413/502 mapping); label text normalization (sanitize.ts, 5 write paths); email/origin trust (fail-closed publicOrigin in production, full HTML+href escaping); image pipeline (one shared decode-validate-reencode for uploads/OFF/OCR, EXIF/GPS/XMP/ICC stripped, animation/SVG/polyglots rejected); external data (OFF exact-host allowlist incl. redirect revalidation, 2 MiB/20s budgets, row shape validation — no more silent truncation of imported evidence); OCR provider output (fresh allowlisted DTO, unknown keys/over-range numbers never reach clients); search/OG budgets (q≤256, ≤12 tokens, page 1–500, degenerate queries → empty shape, OG 30/min/IP).
- Verified (real runs, final): 181 tests / 585 expects (isolated runner + solo mode), eslint clean, tsc clean repo-root, production build green (incl. Windows-portable standalone copy). Production probes after deploy: /api 200 reporting version 0.3.0 + commit d4e8ee6 + db ok; home SSR 200; manifest 200; sw.js v0.3.0 live (content-matched); unknown barcode → real 404; /sok 200; degenerate search query → 200 {"items":[],"total":0,...} (the exact empty-result shape required); /api/og 200 (75KB PNG).
- ROLLBACK: all six commits are additive within Task 30 scope; `git revert d4e8ee6 7ebee90 8bbb18e 7f50468 f23b5a2 e8bf3d0` (newest first) or redeploy the previous Railway deployment snapshot. DB schema unchanged in this release; no migrations accompany it.
- OPERATOR ITEMS (from earlier phases, still open): (1) APP_URL MUST be set in the Railway production environment — publicOrigin now fails closed without it (email sign-in/confirmation links 500 by design); production probes above pass because they don't exercise email links. (2) Caddyfile's XTransformPort query-selected proxy target is a development convenience; deployment usage could not be established from the repo (no Dockerfile/railway.toml checked in) — verify and remove if this Caddyfile is active in production (audit I10). (3) Image concurrency/pixel budgets (40MP/1 decode/queue 8) are named constants — measure against real phone photos + Railway memory and tune. (4) Legacy uploads/text were NOT retroactively normalized (per audit scope) — a read-only inventory + backed-up migration would be a separate task.
- HONEST LIMITS (whole task): normalization is not SQLi/XSS protection in every context — React escaping and Prisma parameterization remain the primary controls in their contexts (both verified intact); no penetration test was performed; no adversarial traffic was sent to production (probes were ordinary requests); dependency advisories were not re-audited after the upgrade (pre-upgrade audit artifacts in docs/audit-2026-09-03/ predate it — follow-up recommended); auth follow-ups (Google email_verified positive evidence, atomic magic-token consumption) remain tracked SEPARATELY per the brief and were not touched; the launch blockers around evidence verification / demo data remain independent of Task 30.
